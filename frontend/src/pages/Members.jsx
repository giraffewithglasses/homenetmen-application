...
import { Plus, Download, Search, Archive } from "lucide-react";
...
export default function Members() {
  ...
  const [form, setForm] = useState({
    ...
    chapter_id: user?.chapter_id || "", section: "Scouts", patrol: "",
  ...
  const load = () => {
    ...
    api.get(`/members?${params}`).then(r => setMembers(r.data));
  };
  ...
  const archive = async (mid) => {
    if (!window.confirm("Archive this member?")) return;
    try { await api.delete(`/members/${mid}`); toast.success("Archived"); load(); }
    catch { toast.error("Failed"); }
  };

  const exportCSV = () => {
    const headers = ["full_name", "email", "phone", "section", "patrol", "chapter_id", "status"];
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">{members.length} shown</p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="btn-pill bg-[hsl(12,65%,63%)]" data-testid="new-member-btn"><Plus size={16} className="mr-2"/>New Member</Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <Button onClick={save} className="btn-pill w-full bg-[hsl(149,40%,30%)] mt-2" data-testid="mbr-save">Save</Button>
            </DialogContent>
          </Dialog>
        </div>
      </div>
            ...
            {members.map((m, i) => (
              <TableRow key={m.member_id} className={i % 2 ? "bg-muted/30" : ""} data-testid={`member-row-${m.member_id}`}>
                ...
                <TableCell className="text-right">
                  <Button size="sm" variant="ghost" onClick={() => archive(m.member_id)} data-testid={`archive-${m.member_id}`}>
                    <Archive size={14} />
                  </Button>
                </TableCell>
              </TableRow>
